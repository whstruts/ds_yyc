...
public interface SpzlMapper {
	...
	@Select("select '同云堂' as suppliers_name,ypdm as ypdm,cddm as cddm,jx as jx,CONVERT(varchar(100), scrq, 23) as scrq," +
			...
			"CONVERT(varchar(100), production_batch, 23) as production_batch,CONVERT(varchar(100), date_expiration, 23) as date_expiration," +
			"repertory as repertory,bar_code as bar_code,suppliers_price as supplier_price,drugid as drugid," +
			"is_on_sale as is_on_sale,category as erp_category from v_ware_b2b_tyt where goods_id_s = #{id} or drugid = #{id}")
	public List<Spbnew> GetKCByIDNEWX(String id);

	@Select("select erpCustomerID AS code,erpCustomerName AS name,cust_type AS type,taxnumber,categories from v_custom_b2b_tyt where RTRIM(LTRIM(taxnumber)) = #{taxnumber} or erpCustomerID = #{taxnumber}")
	...
	@Insert("INSERT INTO d_third_order(order_id,erpCustomerID,Is_pay,count_qty,Total,Create_time,b2b_prom_id,b2b_prom_name,b2b_prom_type,b2b_prom_copuno,b2b_prom_total,b2b_prom_zp,b2b_prom_wx,b2b_prom_zfb,b2b_prom_acount,b2b_prom_yezf,b2b_prom_sjtotal,b2b_prom_ordertotal,Remarks,b2b_offline_pay) " +
			" Values(#{order_id},#{erpCustomerID},#{isPay},#{countQty},#{Total},getdate(),#{b2b_prom_id},#{b2b_prom_name},#{b2b_prom_type},#{b2b_prom_copuno},#{b2b_prom_total},#{b2b_prom_zp},#{b2b_prom_wx},#{b2b_prom_zfb},#{b2b_prom_acount},#{b2b_prom_yezf},#{b2b_prom_sjtotal},#{b2b_prom_ordertotal},#{Remarks},#{b2b_offline_pay})")
